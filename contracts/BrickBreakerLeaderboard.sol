...
contract BrickBreakerLeaderboard {
    struct Player {
    ...
    function submitScore(uint256 _score, string memory _playerName) external {
        ...
        // Add to leaderboard and maintain sorted order
        _insertIntoLeaderboard(msg.sender, _playerName, _score);

        emit ScoreSubmitted(msg.sender, _playerName, _score, block.timestamp);
    }
    
    function _insertIntoLeaderboard(address _player, string memory _playerName, uint256 _score) internal {
        ...
        // If player exists and new score is not higher, don't update leaderboard
        if (existingIndex >= 0 && _score <= leaderboard[uint256(existingIndex)].score) {
            return;
        }

        // If player exists with lower score, remove the old entry
        if (existingIndex >= 0) {
    ...
    function getPlayerRank(address _player) external view returns (uint256) {
        ...
        uint256 rank = 1;
        for (uint256 i = 0; i < leaderboard.length; i++) {
            if (leaderboard[i].score > playerHighScore) {
                rank++;
            } else {
                break;
            }
